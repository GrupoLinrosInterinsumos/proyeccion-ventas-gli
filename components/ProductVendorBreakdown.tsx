          ...
          {rows.map((r) => {
            ...
            const delta =
              r.proyeccion !== null && r.promedio_mensual > 0
                ...
                : null;
            const isOpen = expandedVendedor === r.vendedor;
            return (
              <Fragment key={r.vendedor}>
                <li
                  className="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-surface-container-low"
                  onClick={() => setExpandedVendedor(isOpen ? null : r.vendedor)}
                >
                  <span
